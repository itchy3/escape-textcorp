...
export default async function handler(req, res) {
  ...
  try {
    // ✅ 요청 본문 파싱 (Vercel에서는 req.body 사용)
    const { message, history } = req.body;

    // ✅ OpenAI API 초기화
    const openai = new OpenAI({
      apiKey: "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", // 🔑 여기에 네 키
    });
...
이 프롬프트가 입력되면, 🎯 게임 목표, 룰 요약(아이템 작성 요령, 스탯 및 게이지 요약, 시간제한 등을 설명), 프롤로그 이후에 바로 아이템 작성을 요구하라. ...`;
    ...
    console.log("✅ AI 응답:", reply); // 서버 로그에서 확인 가능
